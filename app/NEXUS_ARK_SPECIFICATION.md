...
## 13. 拡張ツール機能 (Custom Plugins & MCP)

ユーザーが独自の Python プラグインや標準的な MCP (Model Context Protocol) サーバを追加し、AI の機能を動的に拡張できる基盤です。

### 13.1. ローカル Python プラグイン
- **場所:** `custom_tools/` フォルダ
- **仕組み:** `@tool` デコレータを付与した Python 関数を配置することで、AI が自動的に認識し、ツールとして利用可能になります。
- **再スキャン:** UI から動的にリロードが可能です。

### 13.2. MCP (Model Context Protocol) 連携
- **概要:** Anthropic が提唱する標準規格に対応した外部 MCP サーバと連携します。
- **通信:** `stdio` (標準入出力) を介したプロセス間通信。
- **管理:** UI から MCP サーバの登録、削除、および接続テストが行えます。
- **実行環境:** Nexus Ark の Python 仮想環境（venv）を自動的に検出し、依存関係を解決して実行します。

詳細は [拡張ツール機能仕様書](docs/specs/CUSTOM_TOOL_EXTENSION_SPECIFICATION.md) を参照してください。
