...
def handle_refresh_custom_tools():
    ...
    for filename in files:
        ...
        data.append([is_enabled, filename, desc])
...
def handle_mcp_tools_config_change(df_data, selected_info):
    ...
    if not selected_info or not isinstance(df_data, list):
        return
...
def handle_user_gen_instruction_save(selected_name, instruction):
    """テンプレートの保存処理。"""
    if not selected_name:
        return gr.update(), "⚠️ 名前を入力してください。"
    ...
    if not found:
        new_templates.append({"name": selected_name, "instruction": instruction})
        gr.Info(f"テンプレート「{selected_name}」を新規作成しました。")
    else:
        gr.Info(f"テンプレート「{selected_name}」を更新しました。")
    ...
    return gr.update(choices=choices, value=selected_name), ""
...
def handle_user_gen_instruction_delete(selected_name):
    ...
    if len(new_templates) == len(templates):
        return gr.update(), gr.update(), "⚠️ 削除対象が見つかりません。"
    ...
    return gr.update(choices=choices, value=new_val), new_instr, ""
