...
1. ダウンロードしたZIPを、通常のフォルダーへすべて展開します。
2. 展開したフォルダーの `Start.bat` をダブルクリックします。
3. 初回だけ必要なPythonと依存ファイルを自動準備します。黒い画面を閉じずに待ってください。

ZIPの中から直接 `Start.bat` を開いたり、古いNexus Arkのフォルダーへ新版ZIPを上書き展開したりしないでください。
更新中に新版を起動できなかった場合は、保管してある直前の版へ自動で戻します。

#### 以前のWindows版から移行する場合

1. 旧Nexus Arkを終了します。
2. 念のため、旧Nexus Arkフォルダーを別の場所へコピーして保管します。
3. 新版ZIPを**別の新しいフォルダー**へすべて展開します。
4. 新版フォルダーの `UpgradeUpdateSystem.bat` をダブルクリックします。
5. 画面で、これまで使っていた旧Nexus Arkフォルダーを選びます。
6. 「移行が完了しました」と表示されたら、旧フォルダーの `Start.bat` から起動します。

アプリが動作中、更新途中、または安全を確認できない場合、移行ツールは旧 `Start.bat` を変更せず停止します。
その場合は表示された案内を確認し、残っている `update_recovery` やbackupを削除しないでください。
...
- **Python**: 専用のPython 3.12を初回起動時に自動準備します
...
├── UpgradeUpdateSystem.bat ← 旧Windows版からの一度限りの移行用
├── Start.sh        ← Mac/Linux起動用
├── README.md       ← このファイル
├── updater/        ← 起動と旧版復帰を保護する更新機構
└── app/            ← アプリ本体（手動変更不要）
```
