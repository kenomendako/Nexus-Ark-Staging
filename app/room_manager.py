...
import constants
from file_lock_utils import locked_file
...
ROOM_OVERRIDE_STRING_OR_NONE_KEYS = {
    "provider",
    "model_name",
    "api_key_name",
    "zhipu_model",
...
def _sanitize_room_override_updates(room_name: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    """Drop invalid AI string override values before they can overwrite saved settings."""
    sanitized: Dict[str, Any] = {}
    for key, value in updates.items():
        if key in ROOM_OVERRIDE_STRING_OR_NONE_KEYS and value is not None and not isinstance(value, str):
            print(
                f"[room_manager.update_room_config] Warning: ルーム '{room_name}' の "
                f"override_settings.{key} に不正な型 {type(value).__name__} を検出したため保存をスキップします。"
            )
            continue
        sanitized[key] = value
    return sanitized
...
def _restore_missing_monthly_logs_from_backups(abs_base: str) -> List[str]:
    ...
    for month, backup_path in sorted(latest_backup_by_month.items()):
        ...
        clear_marker_path = os.path.join(logs_dir, f".cleared-{month}")
        if os.path.exists(clear_marker_path):
            print(f"--- [REPAIR_LOG] 明示的に全削除された月次ログの自動復元をスキップ: {target_path} ---")
            continue
...
def sync_installed_olivie_official_knowledge() -> Dict[str, Any]:
    """既存Olivieへ公式ガイドを同期し、安全な場合だけ配布用索引も更新する。"""
    source_room = os.path.join(constants.SAMPLE_PERSONA_DIR, "Olivie")
    target_room = os.path.join(constants.ROOMS_DIR, "Olivie")
    source_knowledge = os.path.join(source_room, "knowledge")
    target_knowledge = os.path.join(target_room, "knowledge")
    source_index = os.path.join(source_room, "rag_data", "faiss_index_dynamic")
    target_index = os.path.join(target_room, "rag_data", "faiss_index_dynamic")

    if not os.path.isdir(target_room) or not os.path.isdir(source_knowledge):
        return {"status": "skipped", "reason": "Olivie or bundled knowledge not found"}

    official_files = sorted(
        name for name in os.listdir(source_knowledge) if name.endswith(".md")
    )
    if not official_files:
        return {"status": "skipped", "reason": "bundled official knowledge is empty"}

    digest = hashlib.sha256()
    for filename in official_files:
        digest.update(filename.encode("utf-8"))
        with open(os.path.join(source_knowledge, filename), "rb") as f:
            digest.update(f.read())
    source_model_info = os.path.join(source_index, "model_info.json")
    if os.path.isfile(source_model_info):
        with open(source_model_info, "rb") as f:
            digest.update(f.read())
    bundle_id = digest.hexdigest()

    sync_marker = os.path.join(target_room, "rag_data", "official_knowledge_sync.json")
    if os.path.isfile(sync_marker):
        try:
            with open(sync_marker, "r", encoding="utf-8") as f:
                if json.load(f).get("bundle_id") == bundle_id:
                    return {"status": "unchanged", "bundle_id": bundle_id}
        except (OSError, json.JSONDecodeError):
            pass

    os.makedirs(target_knowledge, exist_ok=True)
    legacy_official_files = {"NEXUS_ARK_SPECIFICATION.md"}
    existing_md = {name for name in os.listdir(target_knowledge) if name.endswith(".md")}
    personal_files = sorted(existing_md - set(official_files) - legacy_official_files)

    for filename in official_files:
        shutil.copy2(
            os.path.join(source_knowledge, filename),
            os.path.join(target_knowledge, filename),
        )
    for filename in legacy_official_files:
        legacy_path = os.path.join(target_knowledge, filename)
        if os.path.isfile(legacy_path):
            os.remove(legacy_path)

    index_updated = False
    needs_rebuild = bool(personal_files)
    if not personal_files and os.path.isdir(source_index):
        ...
        index_updated = True
    elif personal_files and os.path.exists(target_index):
        # 公式ガイドだけの配布索引で個人文書の索引を上書きしない。
        # 古い索引を使い続ける誤検索も避け、UIからの再構築を要求する。
        shutil.rmtree(target_index)
    ...
    try:
        ...
        if not os.path.exists(config_file_path) or os.path.getsize(config_file_path) == 0:
            default_config = {
                ...
                "version": 1,
                "override_settings": dict(constants.NEW_ROOM_DEFAULT_OVERRIDES)
            }
...
def create_backup(room_name: str, file_type: str, new_data: Any = None) -> Optional[str]:
    """
    ...
    try:
        ...
        try:
            with locked_file(source_path, timeout=0.2):
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                if file_type == 'room_config':
                    rotation_count = config_manager.CONFIG_GLOBAL.get("backup_rotation_count", 10)
                    backup_path = create_layered_settings_backup(
                        source_path,
                        backup_dir,
                        original_filename,
            ...
            print(f"警告: バックアップ対象ファイルのロック取得に失敗したためスキップします: {source_path}")
            return None
    ...
    with _room_config_lock:
        ...
        for k, v in updates.items():
            if k in root_keys:
                ...
                safe_updates = _sanitize_room_override_updates(room_name, v)
                deep_merge(overrides, safe_updates)
            else:
                if k in ROOM_OVERRIDE_STRING_OR_NONE_KEYS and v is not None and not isinstance(v, str):
                    print(
                        f"[room_manager.update_room_config] Warning: ルーム '{room_name}' の "
                        f"override_settings.{k} に不正な型 {type(v).__name__} を検出したため保存をスキップします。"
                    )
                    continue
        ...
        # 保存前にバックアップを作成
        try:
            create_backup(room_name, 'room_config', new_data=config)
        except Exception as e:
...
def get_active_working_memory_slot(room_name: str) -> str:
    ...
    try:
        ...
        if config:
            ...
            value = overrides.get("active_working_memory_slot", constants.WORKING_MEMORY_DEFAULT_SLOT)
            # 設定が壊れて非文字列(bool等)が入っていてもクラッシュさせず既定スロットへ自己修復する
...
def restore_log_from_backup(room_name: str, backup_path: str) -> bool:
    ...
    try:
        ...
        # 手動復元は明示的な操作なので、全削除マーカーを解除する。
        import utils
        utils.clear_chat_log_month_marker(log_path_current)
        utils.invalidate_chat_log_cache(log_path_current)
