...
def context_generator_node(state: AgentState):
    ...
    try:
        ...
        if twitter_settings.get("enabled", True):
            ...
            if twitter_manager.is_logged_in():
                ...
                # [NEW] 指針（ガイドライン）の動的注入
                guidelines = twitter_settings.get("posting_guidelines", "").strip()
                if guidelines:
                    twitter_mode_manual_text += "\n\n        【Twitter投稿の指針（必ず遵守すること）】\n"
                    for line in guidelines.split("\n"):
                        if line.strip():
                            twitter_mode_manual_text += f"        - {line.strip()}\n"
    except Exception as e:
    ...
    try:
        ...
        if _tw_settings.get("enabled", True):
            twitter_posting_summary = _tw_settings.get("posting_summary", "").strip()
    except Exception:
        pass
    ...
    for tool in current_tools:
        ...
        # [NEW] ツールの概要（動機付け）を注入
        if tool.name == "draft_tweet" and twitter_posting_summary:
            desc += f"\n  【投稿の目的・目安】: {twitter_posting_summary}"
