...
import datetime
import re
from file_lock_utils import locked_file, safe_text_read

NOTEPAD_LENGTH_WARNING_THRESHOLD = 4000
NOTEPAD_LENGTH_WARNING = (
    "メモ帳が長くなっています。ユーザー宛のメッセージは手紙箱へ、"
    "長期情報は日記やノートへ移すことを検討してください。"
)
...
def read_full_notepad(room_name: str) -> str:
    ...
    content = safe_text_read(notepad_path).strip()
...
def plan_notepad_edit(modification_request: str, room_name: str) -> str:
    ...
    メモ帳は短期の作業メモ専用です。簡潔に残してください。
    ユーザー宛の長文メッセージや通知禁止時間帯の伝言は leave_letter_for_user で手紙箱へ残してください。
    """
...
def _apply_notepad_edits(instructions: List[Dict[str, Any]], room_name: str) -> str:
    ...
    try:
        final_text = ""
        with locked_file(notepad_path) as locked_path:
            lines = locked_path.read_text(encoding="utf-8").split('\n')
            ...
            final_text = "\n".join(new_lines)
            locked_path.write_text(final_text, encoding="utf-8")

        msg = f"成功: {applied_count}件の指示に基づき、メモ帳(notepad.md)を更新しました。"
        if skipped_empty_count > 0:
            msg += f"（※空文字のため{skipped_empty_count}件をスキップしました）"
        if len(final_text) > NOTEPAD_LENGTH_WARNING_THRESHOLD:
            msg += f"\n\n⚠️ {NOTEPAD_LENGTH_WARNING}"
        return msg
