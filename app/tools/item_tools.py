...
@tool
def gift_item_to_user(item_id: str, room_name: str, amount: int = 1) -> str:
    """
    所持しているアイテムを相手に贈ります。
    item_id: 贈るアイテムのID。
    amount: 贈る個数。
...
@tool
def create_food_item(name: str, category: str, base_description: str, room_name: str, amount: int = 1, image_prompt: Optional[str] = None, description: Optional[str] = None) -> str:
    """
...
@tool
def create_standard_item(name: str, category: str, base_description: str, room_name: str, amount: int = 1, image_prompt: Optional[str] = None, description: Optional[str] = None) -> str:
    """
...
def create_and_gift_item(
    ...
    gift_to_user: bool = True,
    description: Optional[str] = None,
) -> str:
    """
    アイテムを作成し、そのまま相手に贈呈する統合ツールです。
    作ったものをすぐプレゼントしたい場合に最適です。

    name: アイテム名（例：「星降るショコラ」「真紅のリボン」）。
    category: カテゴリ（食べ物系：スイーツ、飲み物、料理 等 / 通常：アクセサリー、服飾、雑貨 等）。
    base_description: アイテムの簡単な説明。AIが詳細データを自動生成します。 (エイリアス: description)
    room_name: あなたが現在いるルームの名前。
    item_type: "food"（食べ物）または "standard"（それ以外）。デフォルトは "food"。
    amount: 作成個数（デフォルト: 1）。
    image_prompt: (オプション) 外観画像をAIで生成するための英語の指示文。
    gift_to_user: True なら作成後すぐに相手に贈呈します（デフォルト: True）。
    """
    try:
        # エイリアス対応
        actual_description = base_description or description
        ...
        prompt_text = f"名前: {name}\nカテゴリ: {category}\n詳細: {actual_description}"

        # アイテムタイプに応じてプロファイル生成
        item_type_lower = (item_type or "food").strip().lower()
        if item_type_lower == "food":
            from src.features._recipe_generator import generate_food_item_profile
            json_data = generate_food_item_profile(prompt_text, api_key_val)
        else:
            from src.features._item_desc_generator import generate_standard_item_profile
        ...
        if image_prompt:
            ...
            gen_res = _generate_image_impl(image_prompt, room_name, api_key_val, api_key_name)
            if "[Generated Image:" in gen_res:
                match = _re.search(r"\[Generated Image: (.*?)\]", gen_res)
                if match:
                    generated_image_path = match.group(1).strip()

        im = ItemManager(room_name)
        item_id = im.create_item(json_data, is_user_creator=False, image_path=generated_image_path)
        if not item_id:
            return "エラー: アイテムの作成に失敗しました。"

        res = f"成功: 「{name}」を {amount} 個作成しました (ID: {item_id})。"
        if generated_image_path:
            res += f"\n画像を生成して添付しました: [VIEW_IMAGE: {generated_image_path}]"

        # 贈呈処理
        if gift_to_user:
            transfer_ok = im.transfer_item(item_id, from_user=False)
            if transfer_ok:
                try:
                    import utils
                    utils.append_system_message_to_log(
                        room_name,
                        f"【システム通知】ペルソナからアイテム「{name}」({amount}個) が贈られました。アイテム使用（添付・消費）メニューからインベントリを確認してください。"
                    )
                except Exception as e:
                    logger.error(f"Error appending system message: {e}")
                res += f"\nさらに、相手に贈呈しました。そのことをチャットで伝えてください。"
            else:
                res += f"\n※贈呈処理に失敗しました。手動で gift_item_to_user(item_id='{item_id}') を実行してください。"
        else:
            res += f"\n自分のインベントリに追加されました。贈呈するには gift_item_to_user(item_id='{item_id}') を使用してください。"

        return res
    except Exception as e:
        logger.error(f"Error in create_and_gift_item: {e}")
        return f"エラー: アイテム作成・贈呈処理中に例外が発生しました: {e}"
