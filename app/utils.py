...
SYSTEM_NOTICES: List[Dict[str, Any]] = []

def _normalize_notice_room_name(room_name: Optional[str]) -> Optional[str]:
    """通知対象ルーム名を比較用に正規化する。"""
    if not isinstance(room_name, str) or not room_name.strip():
        return None
    return unicodedata.normalize("NFC", room_name.strip()).casefold()


def add_system_notice(msg: str, level: str = "warning", room_name: Optional[str] = None):
    """
    システム通知を追加する。
    level: "warning", "info", "error"
    room_name: 指定時はそのルームでのみ消費する。未指定は全ルーム共通。
    """
    normalized_room_name = _normalize_notice_room_name(room_name)
    # 同じ本文・対象ルームの重複を避ける。全体通知とルーム通知は別物として保持する。
    if any(
        n["message"] == msg
        and _normalize_notice_room_name(n.get("room_name")) == normalized_room_name
    ...
    target_label = room_name.strip() if isinstance(room_name, str) and room_name.strip() else "all rooms"
    print(f"--- [System Notice Added] {msg} (Level: {level}, Target: {target_label}) ---")

def consume_system_notices(room_name: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    全体通知と指定ルーム宛て通知を返し、対象外ルーム宛て通知は保持する。

    room_name 未指定時は全体通知だけを消費する。ルーム宛て通知を意図せず
    他のバックグラウンド処理が奪わないための安全側の挙動。
    """
    normalized_room_name = _normalize_notice_room_name(room_name)
    consumed: List[Dict[str, Any]] = []
    remaining: List[Dict[str, Any]] = []
    for notice in SYSTEM_NOTICES:
        target_room_name = _normalize_notice_room_name(notice.get("room_name"))
        if target_room_name is None or (
            normalized_room_name is not None and target_room_name == normalized_room_name
        ):
            consumed.append(notice)
        else:
            remaining.append(notice)
    SYSTEM_NOTICES[:] = remaining
    return consumed
