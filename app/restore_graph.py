...
def context_generator_node(state: AgentState):
    ...
    if not state.get("send_scenery", True):
        situation_prompt_parts.append("【現在の状況】")
    ...
    tools_for_prompt = sorted(current_tools, key=lambda tool: getattr(tool, "name", ""))
    tools_list_str = "\n".join([f"- `{tool.name}({', '.join(tool.args.keys())})`: {tool.description}" for tool in tools_for_prompt])
...
def _log_prompt_cache_usage(provider_hint: str, response: AIMessage) -> None:
    try:
        ...
        if cache_read is None and cache_created is None:
            ...
            print(f"  - [Prompt Cache] provider={provider_hint} cache_read_tokens={cache_read or 0}, cache_creation_tokens={cache_created or 0}")
    except Exception as e:
...
def _strip_cache_breakpoint_marker(system_text: str) -> str:
    return system_text.replace(CACHE_BREAKPOINT_MARKER, "")
...
def agent_node(state: AgentState):
    ...
    try:
        ...
        _log_prompt_cache_usage(state.get("model_name", "unknown"), response)
