...
import { getEventsAfterCursor, getMessageRequest, recoverStaleRequests } from "../src/storage";
...
it("期限回収後は帰宅でき、遅延応答が帰宅範囲へ追加されない", async () => {
  const suffix = crypto.randomUUID();
  const raw = snapshot(suffix);
  raw.session_budget.allow_unknown_price = true;
  raw.personas[0]!.initial_route = {credential_profile_id: "gemini-personal-1", model_id: "model-phase1"};
  ...
  const pending = streamTravelMessage(env.DB, env, raw.travel_session_id, {
    client_message_id: "expiry-return-" + suffix, persona_id: raw.personas[0]!.persona_id, message: "Synthetic delayed",
  }, (async () => {
    reached();
    await gate;
    return new Response(geminiNormal, {headers: {"Content-Type": "text/event-stream"}});
  }) as typeof fetch, () => new Date("2026-09-14T00:00:00.000Z"));
  await providerStarted;
  await expect(startReturn(env.DB, raw.travel_session_id, "2026-09-14T00:29:00Z")).rejects.toThrow("session_message_in_progress");
  expect(await recoverStaleRequests(env.DB, "2026-09-14T00:30:00.000Z")).toBe(1);
  const manifest = await startReturn(env.DB, raw.travel_session_id, "2026-09-14T00:31:00Z");
  expect(manifest.personas.map((persona) => persona.high_water_sequence)).toEqual([0, 0, 0]);
  release();
  expect(await (await pending).text()).not.toContain("response.committed");
  expect((await getMessageRequest(env.DB, "expiry-return-" + suffix))?.status).toBe("outcome_unknown");
  const chunk = await buildSignedReturnChunk(env.DB, raw.travel_session_id, raw.personas[0]!.persona_id,
    0, "phase1-signing-test-key", "2026-09-14T00:32:00Z");
  expect(chunk.payload.cursor).toEqual({after_sequence: 0, through_sequence: 0});
});

it("明示cacheの応答本文が終わらなくても期限後に生成を始めない", async () => {
  const suffix = crypto.randomUUID();
  const base = snapshot(suffix);
  const raw = {...base, personas: base.personas.map((persona) => ({
    ...persona, cache_policy: "gemini_explicit" as const,
    recent_messages: [{role: "user" as const, content: "Synthetic stable cache context"}],
  }))};
  raw.session_budget.allow_unknown_price = true;
  raw.personas[0]!.initial_route = {credential_profile_id: "gemini-personal-1", model_id: "model-phase1"};
  await registerSnapshot(env.DB, raw);
  ...
  let aborted = false;
  const response = await streamTravelMessage(env.DB, env, raw.travel_session_id,
    {client_message_id: "cache-timeout-" + suffix, persona_id: raw.personas[0]!.persona_id, message: "Synthetic cache timeout"},
    (async (input: RequestInfo | URL, init?: RequestInit) => {
      urls.push(String(input));
      if (String(input).includes("countTokens")) return Response.json({totalTokens: 40000});
      return new Response(new ReadableStream({
        start(controller) {
          init?.signal?.addEventListener("abort", () => {
            aborted = true;
            controller.error(new Error("synthetic aborted cache body"));
          }, {once: true});
        },
      }), {headers: {"Content-Type": "application/json"}});
    }) as typeof fetch, () => new Date(), 100);
  expect(response.status).toBe(502);
  expect(await response.text()).toContain("provider_outcome_unknown");
  expect(urls).toHaveLength(2);
  expect(urls[1]).toContain("cachedContents");
  expect(aborted).toBe(true);
  expect((await getMessageRequest(env.DB, "cache-timeout-" + suffix))?.status).toBe("outcome_unknown");
});
