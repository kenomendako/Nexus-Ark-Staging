...
import { getEventsAfterCursor, getMessageRequest, markProviderStarted, recoverStaleRequests, reserveMessage } from "../src/storage";
...
describe("Phase 1 Gemini travel chat", () => {
  it.each(["", "\n"])("SSEのEOF区切り%jでも表示全文とatomic確定内容を一致させる", async (ending) => {
...
describe("provider deadline and late response", () => {
  for (const phase of ["fetch", "stream"]) {
    it("終わらない" + phase + "を期限で打ち切り、同じ要求を再送しない", async () => {
      const suffix = crypto.randomUUID();
      const raw = snapshot(suffix);
      await registerSnapshot(env.DB, raw);
      const input = {client_message_id: "deadline_" + suffix, message: "Synthetic timeout"};
      let calls = 0;
      ...
      const fetcher = (async (_input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (phase === "fetch") return await new Promise<Response>(() => {});
        return new Response(new ReadableStream({
          cancel() { streamCancelled = true; },
        }), {headers: {"Content-Type": "text/event-stream"}});
      }) as typeof fetch;
      const response = await streamGeminiMessage(env.DB, env, raw.travel_session_id, input,
        fetcher, () => new Date(), 100);
      const body = await response.text();
      expect(body).toContain(phase === "fetch" ? "provider_outcome_unknown" : "stream_interrupted");
      expect(signals[0]?.aborted).toBe(true);
      if (phase === "stream") expect(streamCancelled).toBe(true);
      expect(await getMessageRequest(env.DB, input.client_message_id)).toMatchObject({status: "outcome_unknown"});
      expect((await getEventsAfterCursor(env.DB, raw.travel_session_id, raw.persona_id, 0)).events).toHaveLength(0);
      const duplicate = await streamGeminiMessage(env.DB, env, raw.travel_session_id, input, fetcher);
      expect(duplicate.status).toBe(409);
      expect(calls).toBe(1);
    });
  }

  for (const lateStatus of [200, 400]) {
    it("回収後の遅延HTTP " + lateStatus + "が確定eventや予約額を巻き戻さない", async () => {
      const suffix = crypto.randomUUID();
      const raw = snapshot(suffix);
      await registerSnapshot(env.DB, raw);
      const input = {client_message_id: "late_" + suffix, message: "Synthetic delayed response"};
      let release!: () => void;
      let reached!: () => void;
      const gate = new Promise<void>((resolve) => { release = resolve; });
      const providerStarted = new Promise<void>((resolve) => { reached = resolve; });
      const pending = streamGeminiMessage(env.DB, env, raw.travel_session_id, input, (async () => {
        reached();
        await gate;
        return new Response(lateStatus === 200 ? geminiNormal : "synthetic refusal",
          {status: lateStatus, headers: {"Content-Type": "text/event-stream"}});
      }) as typeof fetch, () => new Date("2026-09-14T00:00:00.000Z"));
      await providerStarted;
      await env.DB.prepare("UPDATE message_requests SET budget_state = 'reserved', budget_reserved_usd = 0.1 WHERE client_message_id = ?").bind(input.client_message_id).run();
      await recoverStaleRequests(env.DB, "2026-09-14T00:30:00.000Z");
      release();
      const response = await pending;
      const body = await response.text();
      expect(body).not.toContain("response.committed");
      expect(await getMessageRequest(env.DB, input.client_message_id)).toMatchObject({
        status: "outcome_unknown", budget_state: "held", budget_reserved_usd: 0.1, budget_settled_usd: null,
      });
      expect((await getEventsAfterCursor(env.DB, raw.travel_session_id, raw.persona_id, 0)).events).toHaveLength(0);
      const receipts = await env.DB.prepare("SELECT COUNT(*) AS n FROM usage_receipts WHERE travel_session_id = ?").bind(raw.travel_session_id).first<{n: number}>();
      expect(receipts?.n).toBe(0);
      if (lateStatus === 200) {
        const captures: Array<Record<string, unknown>> = [];
        const next = await streamGeminiMessage(env.DB, env, raw.travel_session_id,
          {client_message_id: "next_" + suffix, message: "Synthetic new question"}, provider(geminiNormal, captures));
        expect(await next.text()).toContain("response.committed");
        expect(captures).toHaveLength(1);
        expect(await getMessageRequest(env.DB, input.client_message_id)).toMatchObject({
